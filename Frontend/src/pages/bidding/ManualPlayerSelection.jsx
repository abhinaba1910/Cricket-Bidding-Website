...
//   const [savailablePlayers, setSavailablePlayers] = useState([]);
...
//       state: {
//         savailablePlayers: savailablePlayers.map((p, i) => ({
//           ...p,
//           selectionOrder: i + 1
//         }))
//       }
...
//               {filtered.map(p => {
//                 const isSel = savailablePlayers.some(x => x._id === p._id);
//                 const idx = isSel ? savailablePlayers.findIndex(x => x._id === p._id) + 1 : null;
//                 return (
//                   <tr
//                     key={p._id}
...
//             disabled={savailablePlayers.length === 0}
//             className={`px-6 py-3 rounded-lg font-medium transition ${
//               savailablePlayers.length === 0
//                 ? "bg-gray-300 text-gray-500 cursor-not-allowed"
//                 : "bg-green-600 text-white hover:bg-green-700"
//             }`}
//           >
//             Start Bidding ({savailablePlayers.length})
//           </button>
//         </div>
//       </div>
//     </div>
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        setSavailablePlayers(savailablePlayers || []);
      })
          ...
          <table className="w-full table-fixed text-left divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((p) => {
                const isSel = savailablePlayers.some((x) => x._id === p._id);
                const idx = isSel
                  ? savailablePlayers.findIndex((x) => x._id === p._id) + 1
                  : null;
                return (
                  <tr
                    key={p._id}
