    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 max-md:px-0 max-md:pb-14">
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(p => (
              <tr key={p._id} className="hover:bg-gray-50 transition">
                <td className="px-4 py-2">
                  <img src={p.playerPic || 'https://placehold.co/60x60'} alt={p.name} className="w-10 h-10 rounded-full" />
                </td>
      ...
      <div className="md:hidden bg-white rounded-lg shadow overflow-x-auto">
        <table className="min-w-full text-left divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(p => (
              <tr key={p._id} className="odd:bg-gray-50 even:bg-white hover:bg-gray-100 transition rounded-lg">
                <td className="px-3 py-4">
                  <p className="font-medium text-gray-900">{p.name}</p>
                  <p className="text-sm text-gray-600 mt-1">{p.role}</p>
                </td>
                <td className="px-3 py-4 text-sm space-y-1">
                  <div className="inline-block px-2 py-1 bg-indigo-100 rounded-full text-indigo-800">{p.grade || '—'}</div>
                  <div>{`₹${p.basePrice?.toLocaleString() || 0}`}</div>
                </td>
                <td className="px-3 py-4 flex space-x-2 justify-end">
                  <a href={`/players/${p._id}`} className="flex items-center p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition">
                    <FiEye size={16} />
                  </a>
                  <a href={`/players/${p._id}/edit`} className="flex items-center p-2 bg-blue-100 rounded-full hover:bg-blue-200 transition">
                    <FiEdit size={16} />
                  </a>
                </td>
              </tr>
