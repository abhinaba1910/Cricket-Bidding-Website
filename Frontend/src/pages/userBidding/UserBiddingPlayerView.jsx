  ...
  useEffect(() => {
    const fetchPlayer = async () => {
      try {
        ...
        console.log("User Player data with performance stats:", res.data);
        setPlayer(res.data);
      } catch (err) {
            ...
            {performanceStats && (
              ...
              >
                ...
                <div className="space-y-6">
                  ...
                  {(role === "Fast all-rounder" || role === "Spin all-rounder") && performanceStats.allRounder && (
                    <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl p-4 border border-purple-100">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        <PerformanceStat
                          label="Batting Avg"
                          value={performanceStats.allRounder.battingAverage?.toFixed(1) || "0.0"}
                          icon="📊"
                        />
                        <PerformanceStat
                          label="Strike Rate"
                          value={performanceStats.allRounder.battingStrikeRate?.toFixed(1) || "0.0"}
                          icon="🎯"
                        />
                        <PerformanceStat
                          label="Centuries"
                        ...
                        <PerformanceStat
                          label="Best Bowling"
                          value={performanceStats.allRounder.bestBowling || "0/0"}
                          icon="⭐"
                        />
                        <PerformanceStat
                          label="Bowl Avg"
                          value={performanceStats.allRounder.bowlingAverage?.toFixed(1) || "0.0"}
                          icon="📈"
                        />
                        <PerformanceStat
                          label="Economy"
                          value={performanceStats.allRounder.economy?.toFixed(1) || "0.0"}
                          icon="💰"
                        />
                        <PerformanceStat
                          label="5-Wickets"
                  ...
                  {(role === "Batsman" || role === "Wicket keeper batsman") && performanceStats.batting && (
                    <div className="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl p-4 border border-orange-100">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        <PerformanceStat
                          label="Average"
                          value={performanceStats.batting.average?.toFixed(1) || "0.0"}
                          icon="📊"
                        />
                        <PerformanceStat
                          label="Strike Rate"
                          value={performanceStats.batting.strikeRate?.toFixed(1) || "0.0"}
                          icon="🎯"
                        />
                        <PerformanceStat
                          label="Centuries"
                  ...
                  {(role === "Fast bowler" || role === "Spin bowler") && performanceStats.bowling && (
                    <div className="bg-gradient-to-r from-green-50 to-teal-50 rounded-xl p-4 border border-green-100">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        <PerformanceStat
                          label="Best Bowling"
                          value={performanceStats.bowling.bestBowling || "0/0"}
                          icon="⭐"
                        />
                        <PerformanceStat
                          label="Average"
                          value={performanceStats.bowling.average?.toFixed(1) || "0.0"}
                          icon="📊"
                        />
                        <PerformanceStat
                          label="Economy"
                          value={performanceStats.bowling.economy?.toFixed(1) || "0.0"}
                          icon="💰"
                        />
                        <PerformanceStat
                          label="5-Wickets"
                  ...
                  {!((role === "Fast all-rounder" || role === "Spin all-rounder") && performanceStats.allRounder) &&
                   ...
                   !((role === "Fast bowler" || role === "Spin bowler") && performanceStats.bowling) && (
                    <div className="text-center py-8 text-gray-500">
                      <FiBarChart size={48} className="mx-auto mb-4 text-gray-300" />
                      <p className="text-lg font-medium">No performance statistics available</p>
                      <p className="text-sm">Statistics for {role} will be displayed once match data is available.</p>
                    </div>
                  )}
                </div>
              </motion.div>
